...
typedef volatile struct __attribute__((packed)){
    ...
    const uint32_t DITARGETRSO[8]; /* 0x800 - 0x81C Interrupt CPU targets, RO */
    ...
    /* Some PPI, SPI status registers and identification regsisters beyond this 
...
typedef volatile struct __attribute__((packed)){
    ...
    const uint32_t CHPPIR;      /* 0x18 higher priority pending register */
    uint32_t CABPR;             /* 0x1C Aliased binary point */
    const uint32_t CAIAR;       /* 0x20 Aliased interrupt */

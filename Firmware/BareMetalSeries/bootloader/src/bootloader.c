...
static void aes_cbc_mac_step(AES_Block_t aes_state, AES_Block_t prev_state, const AES_Block_t *keySchedule) {
  // The CBC chaining operation
  for (uint8_t i = 0; i < AES_BLOCK_SIZE; i++) {
    ((uint8_t*)aes_state)[i] ^= ((uint8_t*)prev_state)[i];
  }

  AES_EncryptBlock(aes_state, keySchedule);
...
static bool is_device_id_packet(const comms_packet_t* packet){
  if(packet->length != 2){
    ...
    if (packet->data[0] != BL_PACKET_DEVICE_ID_REQ_DATA0){
        return false;
    }
